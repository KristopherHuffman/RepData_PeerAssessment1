...
library(tidyverse)
library(lattice)
options(scipen = 999) # turn off scientific notation output
...
The mean number of steps taken per day is `r round(summary(steps$stepsperday)[4],1)` and the median number of steps taken per day is `r round(summary(steps$stepsperday)[3],1)`.
...
Interval `r int5$interval[which(int5$avgSteps==max(int5$avgSteps))]` is the 5-minute interval that contains the maximum number of average steps.
...
In total, there are `r nrow(miss)` observations with missing values.

For the imputed data, the mean number of steps taken per day is `r round(summary(steps$stepsperday)[4],1)` and the median number of steps taken per day is `r round(summary(steps$stepsperday)[3],1)`. These values are virtually identical to those obtained by simply deleting missing observations. The impact of imputation on mean and median calculations is negligible.
